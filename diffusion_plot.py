...
start = time.time()
reducer = dm.DiffusionMap.from_sklearn(n_evecs=3, alpha=0.5, epsilon='bgh')
embedding = reducer.fit_transform(X_trans)
logger.info(f"Diffusion completed in {time.time() - start:.2f} seconds.")

# Save embedding
emb_df = pd.DataFrame(embedding, columns=["UMAP_1", "UMAP_2"])
emb_df.to_csv(f"diffusion_embeddings_{pct}_sample.csv", index=False)
del emb_df
gc.collect()
...
plt.title(f"Diffusion with Density Coloring ({pct}%)", fontsize=14)
plt.axis("off")
plt.tight_layout()
