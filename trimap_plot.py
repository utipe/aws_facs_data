...
# --- Logging Setup ---
log_file = os.path.join(os.path.dirname(__file__), "trimap_run.log")
logging.basicConfig(
    filename=log_file,
    filemode='a',
    level=logging.INFO,
...
logger.info(f"Trimap completed in {time.time() - start:.2f} seconds.")

# Save embedding
emb_df = pd.DataFrame(embedding, columns=["trimap_1", "trimap_2", "trimap_3"])
emb_df.to_csv(f"trimap_embeddings_{pct}_sample.csv", index=False)
del emb_df
gc.collect()
...
plt.title(f"TRIMAP with Density Coloring ({pct}%)", fontsize=14)
plt.axis("off")
plt.tight_layout()
plt.savefig(f"TRIMAP{pct}_sample.png", dpi=300)
plt.close()
